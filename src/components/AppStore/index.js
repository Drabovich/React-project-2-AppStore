...
class AppStore extends Component {
  ...
  render() {
    ...
    const filterSearch = appsList.filter(each =>
      each.appName.toLowerCase().includes(searchInput),
    )
    const categoryFilter = filterSearch.filter(
        ...
        <div className="app-store">
          ...
          <div className="tabs-list">
            {tabsList.map(each => (
              <TabItem
                tabsListItem={each}
                categoryItem={this.updateCategory}
                key={each.tabId}
              />
            ))}
          </div>
          <div className="apps-list">
            {categoryFilter.map(eachApp => (
              <AppItem key={eachApp.appId} eachAppItems={eachApp} />
            ))}
          </div>
        </div>
      </div>
